function [O,S]=BungeUARS(k,a,b,g,n)
...
for c=1:length(rvec)
qr=(1-cos(rvec(c)))/(2*pi)*exp(-k^2*(rvec(c))^2/2);
...
u2=z2/zd;
u3=z3/zd;
%by rejection sampling to generate r from Bunge Density, the proposal is
%wmb circular density

rn=rwmb(k,1);
x=rand;
while x>=(1-cos(rn))/(2*pi)*exp(-k^2*(rn)^2/2)/(aa*densitywmb(k,rn))
    x=rand;
    rn=rwmb(k,1);
end;
r=rn;
