function [O,S]=CayleyUARS(k,a,b,g,n)
...
u2=z2/zd;
u3=z3/zd;
%by rejection sampling to generate r from cayley, the proposal is wmb
rn=rwmb(k,1);
x=rand;
while x>=densitycayley(k,rn)/(aa*densitywmb(k,rn))
    x=rand;
    rn=rwmb(k,1);
end;
r=rn;
