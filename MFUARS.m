function [O,S]=MFUARS(k,a,b,g,n)
%rejection sampling, proposal is changed to wmb, previous proposal is
%vonmises
...
u2=z2/zd;
u3=z3/zd;
%by rejection sampling to generate r from MF, the proposal is wmb
rn=rwmb(k,1);
x=rand;
while x>=densityMF(k,rn)/(aa*densitywmb(k,rn))
    x=rand;
    rn=rwmb(k,1);
end;
r=rn;
