function [O,S]=LorentzianUARS(k,a,b,g,n)
...
u2=z2/zd;
u3=z3/zd;
%by rejection sampling to generate r from Lorentzian, the proposal is uniform
rn=-pi+2*pi*rand;
x=rand;
while x>=densitylorentz(k,rn)/(aa/(2*pi))
    x=rand;
    rn=-pi+2*pi*rand;
end;
r=rn;
